<template>
    <div class="destinations">
        <div class="list" v-if="forDestinationsList.length > 0">
            <UCard v-for="value in forDestinationsList" :key="value.$id">
                <template #header>
                    <h3>{{ value.title }}</h3>
                </template>
        ...
        <empty-section section="Destinations" v-else />
        <div class="addButton">
            <UButton @click="adminComponent = 'Add Destinations'" icon="solar:add-circle-bold-duotone" label="Add New Destination" />
        </div>

        <UModal v-model:open="isDeleteModal" :ui="{ footer: 'justify-end' }">
            <template #header>
                <div class="to">
                    <h3>Delete Destinations</h3>
