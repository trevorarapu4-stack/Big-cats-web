<template>
    <div class="categories">
        <div class="list" v-if="forCategoriesList.length > 0">
            <UCard v-for="value in forCategoriesList" :key="value.$id">
                <template #header>
                    <h3>{{ value.title }}</h3>
                </template>
        ...
        <empty-section section="Categories" v-else />
        <div class="addButton">
            <UButton @click="adminComponent = 'Add Category'" icon="solar:add-circle-bold-duotone"
        ...
        <UModal v-model:open="isDeleteModal" :ui="{ footer: 'justify-end' }">
            <template #header>
                <div class="to">
                    ...
                    <p>Would you like to delete the categories {{ docToDelete?.title }}.</p>
                </div>
            </template>
