<template>
    <div class="singleTour">
        ...
        <div class="inner">
            <h1>{{ forBlogDoc?.title }}</h1>
            <p>Written By Big Cat Safaris</p>
            <div class="bb">
                <u-button icon="solar:square-share-line-bold-duotone" variant="outline" label="Share this blog" />
            </div>
            <img :src="forBlogDoc?.image" :alt="forBlogDoc?.title">
            <div class="bCont" v-html="getHtml(forBlogDoc?.content)"></div>
        </div>
    </div>
</template>
